...
public class SubjectDaoHibernate implements SubjectDao {
	...
	private final static Logger log = LogManager.getLogger(SubjectDaoHibernate.class.getName());
	...
	public List<Subject> findSubjectsWithoutDepartment() throws DaoException {
		...
		try (Session session = sessionFactory.openSession()) {
			subjects = (List<Subject>) session
					.createQuery(
							"from Subject s where not exists (from Department d where s in elements(d.subjects))")
					.list();
		}
